...
Overview
This project is a backend service that parses appointment requests from natural language text or scanned images and converts them into structured appointment data. It integrates OCR, entity extraction, date/time normalization, and exposes a REST API.
Features
...
text	string	Appointment request in text
file	file	Image file containing request
...
│   ├── normalize.py     # Date/time normalization logic
...
2. Basic code snippets for your project modules
a) app/ocr.py — OCR using pytesseract
...
b) app/extract.py — Extract entities (date, time, department) from text
import re

def extract_entities(text: str):
    # Simple regex based extraction (you can improve this with NLP)
...
c) app/normalize.py — Normalize date/time to ISO format in Asia/Kolkata timezone
import dateparser
...
d) app/main.py — FastAPI app with /parse endpoint
from fastapi import FastAPI, UploadFile, File, Query
from app.ocr import extract_text_from_image
from app.extract import extract_entities
from app.normalize import normalize_entities

app = FastAPI()

@app.post("/parse")
async def parse_appointment(text: str = Query(None), file: UploadFile = File(None)):
    raw_text = ""
    if file:
